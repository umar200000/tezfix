        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map((service) => {
              const cats = JSON.parse(service.servicesList) as string[];
              return (
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex flex-wrap gap-1">
                      {cats.slice(0, 2).map((c) => (
                        <span key={c} className="px-2 py-0.5 bg-primary-50 text-primary-700 rounded text-xs">{c}</span>
                      ))}
                      {cats.length > 2 && (
                        <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">+{cats.length - 2}</span>
                      )}
                    </div>
                  </td>
                </tr>
